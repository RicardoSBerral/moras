...
/*************************************************************************/
/*									 */
/*  Pues eso, libera un poco de memoria                       		 */
/*									 */
/*************************************************************************/
void    ReleaseNames(void)
{
    ...
    for(i=0;i<=MaxAtt;i++) {
        free(AttName[i]);
        for(j=1;j<=MaxAttVal[i];j++) {
//printf("%s %x\n", AttValName[i][j], (unsigned int)AttValName[i][j]);
            free(AttValName[i][j]);
        }
        free(AttValName[i]);
    }
   
    for(i=0;i<=MaxClass;i++) free(ClassName[i]);
...
//printf("AttValName: %s\n", Buffer);
	...
	if ( MaxAttVal[MaxAtt] == 1 )
	{
		...
		AttValName[MaxAtt][0] = (char *) ((int)v);
	    ...
	    free(AttValName[MaxAtt][1]);//Memory leak fix (18-12-03)
	    MaxAttVal[MaxAtt] = 0;
	}
	else
