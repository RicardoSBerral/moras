...
{ 
	...
	LocalVerbosity(1)
	{
        ...
        ForEach(v, 1, T->Forks) //Memory leak fix (GMM 18-12-03)
	{
	    ReleaseTree(T->Branch[v]);
	}
	free(T->Branch);
	T->Branch = 0;
    ...
    {
	Tree *br = T->Branch;  //Memory leak fix (GMM 18-12-03)
	LocalVerbosity(1)
	{
	    Intab(Sh);
	...
	EstimateErrors(T->Branch[MaxBr], Fp, Lp, Sh, true);
        free(T->ClassDist); //Memory leak fix (GMM 18-12-03)
        ForEach(v, 1, T->Forks) //Memory leak fix (GMM 18-12-03)
	{
	    if (v==MaxBr) continue;
	    ReleaseTree(T->Branch[v]);
	...
	memcpy((char *) T, (char *) T->Branch[MaxBr], sizeof(TreeRec));
        free(br[MaxBr]);           //Memory leak fix (GMM 18-12-03)
        free(br);           //Memory leak fix (GMM 18-12-03)
	Changed = true;
    }
    else
    {
