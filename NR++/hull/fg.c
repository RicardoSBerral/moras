...
static int fg_vn_visit_fg;
void visit_fg(fg *faces_gr, void (*v_fg)(Tree *, int, int)) {
//	static int fg_vn_visit_fg;
	visit_fg_i(v_fg, faces_gr->facets, 0, ++fg_vn_visit_fg, 1);
}
...
static int fg_vn_visit_fg_far;
void visit_fg_far(fg *faces_gr, void (*v_fg)(Tree *, int)) {
//	static int fg_vn_visit_fg_far;
	visit_fg_i_far(v_fg,faces_gr->facets, 0, --fg_vn_visit_fg_far);
}
...
//	static double mults[MAXDIM];
	...
	fa_p_fg[depth] = site_num(t->key);
	for (i=0;i<=depth;i++)
		fprintf(FG_OUT, "%d ", fa_p_fg[i]);
...
void dejamostodocomoestaba_fg() {
	...
	for(i=0;i<100;i++)
	    for(j=0;j<100;j++)
		fg_hist[i][j] = fg_hist_bad[i][j] = fg_hist_far[i][j] = 0.0;

	fg_vn_visit_fg = 0;
	fg_vn_visit_fg_far = 0;
