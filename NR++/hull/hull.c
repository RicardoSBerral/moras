...
#define push_1(x) *(st_1+tms++) = x;
#define pop_1(x)  x = *(st_1 + --tms);

static long vnum = -1;
static long ss_1 = 2000;
static simplex **st_1;
...
//	static simplex **st_1;
	...
	vnum--;
	if (!st_1) assert(st_1=(simplex**)malloc((ss_1+MAXDIM+1)*sizeof(simplex*)));
	if (s) push_1(s);
	while (tms) {

		if (tms>ss_1) {DEBEXP(-1,tms);
			assert(st_1=(simplex**)realloc(st_1,
				((ss_1+=ss_1)+MAXDIM+1)*sizeof(simplex*)));
		}
		pop_1(t);
...
#define push_2(x) *(st_2+tms++) = x;
#define pop_2(x)  x = *(st_2 + --tms);
...
//	static long ss = MAXDIM;
	...
	if (!st_2) st_2 = (simplex **)malloc((ss+MAXDIM+1)*sizeof(simplex*));
	push_2(root->peak.simp);
	root->visit = pnum;
	if (!sees(p,root))
		for (i=0,sn=root->neigh;i<cdim;i++,sn++) push_2(sn->simp);
	while (tms) {
		if (tms>ss) 
			assert(st_2=(simplex**)realloc(st_2,
		...
		for (i=0, sn=s->neigh; i<cdim; i++,sn++) push_2(sn->simp);
	}
	return NULL;
}
...
void dejamostodocomoestaba_hull(void){
	p = 0;
	pnum = 0;
