...
void dejamostodocomoestaba(void) {
	//Dejamos todo como estaba y nos cagamos de paso
	//en las variables globales y estAticas
	int i;
	for(i=0;i<num_blocks;i++) {
		free(site_blocks[i]);
		site_blocks[i] = 0;
	...
	dejamostodocomoestaba_ch();
	dejamostodocomoestaba_fg();
	dejamostodocomoestaba_hull();
}	

void mainnn(int argc, char **argv) {
...
/*	fprintf(DFILE,"dim=%d\n",dim);fflush(DFILE); */
	...
	free_hull_storage();
	dejamostodocomoestaba();
	
//	exit(0);
}

int *convexhull(double **pts, int npts, int dim, double mult) 
{
  ...
  argv[0] = (char*)malloc(sizeof(char)*300);
  argv[1] = (char*)malloc(sizeof(char)*300);
  argv[2] = (char*)malloc(sizeof(char)*300);
  argv[3] = (char*)malloc(sizeof(char)*300);
  
  //Se crean nombres de ficheros temporales para entrada y salida
  strcpy(ni, "./XXXXXX");
  mkstemp(ni);
  strcpy(no, "./XXXXXX");
  mkstemp(no);
  ...
  strcpy(argv[0], "hull");
  sprintf(argv[1], "-m %lf", mult);
  sprintf(argv[2], "-i%s", ni);
  sprintf(argv[3], "-oF%s", no);
  ...
  int nalloc = 100, ires=1;
  int *res = (int*)malloc(sizeof(int)*nalloc);
  res[0] = 0;
  char buf[1024], *s;
  f=fopen(no, "rt");
  while((s=fgets(buf, sizeof(buf), f))) {
    if (buf[0]=='%') continue;
    if (nalloc<ires+dim+1) {
      nalloc *= 2;
      res = (int*)realloc(res, sizeof(int)*nalloc);
    }
    s = strtok(s, " \t\n");
    while(s) {
      sscanf(s, "%d", &res[ires++]);
