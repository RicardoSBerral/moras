...
static neighbor p_neigh_init_value={0,0,0};
...
//	static basis_s *b_cp = NULL;
	...
	if (!b_cp) assert(b_cp = (basis_s*)malloc(basis_s_size));
	else b_cp->lscale = 0;
	z = VB(b_cp);
	tt = s->neigh[0].vert;
	for (i=1;i<cdim;i++) {
		y = s->neigh[i].vert;
		if (vd && y==infinity) memcpy(b_cp, infinity_basis, basis_s_size);
		else {trans(z,y,tt); lift(z,s);}
		if (s->normal && cosangle_sq(b_cp,s->normal)>b_err_min_sq) {DEBS(0)
		...
		for (j=i+1;j<cdim;j++) {
			if (cosangle_sq(b_cp,s->neigh[j].basis)>b_err_min_sq) {
				DEBS(0)
				DEB(0,bad basis)DEBEXP(0,i) DEBEXP(0,j) DEBEXP(0,dd)
				DEBTR(-8) print_basis(DFILE, b_cp);
				print_simplex_f(s, DFILE, &print_neighbor_full);
				EDEBS
				return 0;
			}
...
//	static basis_s *b_sees = NULL;
	...
	if (!b_sees) assert(b_sees = (basis_s*)malloc(basis_s_size));
	else b_sees->lscale = 0;
	zz = VB(b_sees);
	if (cdim==0) return 0;
	if (!s->normal) {
		get_normal_sede(s);
	...
	tt = s->neigh[0].vert;
	if (vd) {
		if (p==infinity) memcpy(b_sees,infinity_basis,basis_s_size);
...
//	static neighbor *sn_v;
	...
	if (!s_v) {NEWL(simplex,s_v); sn_v = s_v->neigh;}
	cdim = depth;
	s_v->normal = n;
	if (depth>1 && sees(t->key,s_v)) signum = -1; else signum = 1;
	cdim = tdim;

	if (t->fgs->dist == 0) {
		sn_v[depth-1].vert = t->key;
		NULLIFY(basis_s,sn_v[depth-1].basis);
		cdim = depth; get_basis_sede(s_v); cdim = tdim;
		reduce(&nn, infinity, s_v, depth);
		nnv = nn->vecs;
...
static basis_s	tt_basis_init_value = {0,1,-1,0,0,0};
void dejamostodocomoestaba_ch(){
	int i;
	...
	basis_vec_size = 0;
        exact_bits = 0;
	tt_basis = tt_basis_init_value;
	tt_basisp = &tt_basis;
	infinity_basis = 0;
	infinity[0] = 57.2;
	for(i=1;i<10;i++) infinity[i] = 0;
	for(i=0;i<100;i++)
		A[i] = B[i] = C[i] = D[i] = 0;
	tot = 0;
	totinf = 0;
	...
	for(i=0;i<MAXPOINTS;i++)
	        mi[i] = mo[i] = 0;
        b_err_min = b_err_min_sq = 0.0;
	get_site = 0;
	site_num = 0;

	//Variables estAticas locales hechas globales
	lscale = 0;
	max_scale = 0.0;
	failcount = 0;
	if (p_neigh.basis) free(p_neigh.basis);
	p_neigh = p_neigh_init_value;
	if (b_cp) free(b_cp);
	b_cp = NULL;
	if (b_sees) free(b_sees);
	b_sees = NULL;
	alpha = 0.0;
	s_v = 0;
	sn_v = 0;
