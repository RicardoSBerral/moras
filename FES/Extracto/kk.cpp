...
NomData::NomData(DataSet* ds/*1, CConsole* pt*/,void(*_console)(char*))
  : Data(ds/*1,pt*/,_console)
{
...
//End GMM
  {
    ...
    if ((aux=(int)m[i][Nvar-1])+1 > NumClass) //  Assume format for class is an integer larger than 0
      NumClass = aux+1;
  }
...
/*    char puf[128];
  ...
  */}
  ...
  {
    n->RPrune = 0;
    n->firstTest = Ndata; n->lastTest = Ndata+ NPrune - 1;
    FindMembershipTest(n);
    for(int i = n->firstTest; i <= n->lastTest; ++i)
    {
...
void NomData::FixCVError(Node* root)
{
  ...
  cur->RleafCV = 0.0;
  cur->firstTest = Ndata; cur->lastTest = NGrow-1;
  ...
  while(cur)
  {
    ...
    if(cur->CrispFlag)
    {
      ...
      for(int i=cur->firstTest;i<=cur->lastTest;i++)
      {  

        if((int)m[i][Nvar-1] != cur->iClass)
          cur->RleafCV += m[i][Nvar+4];  
      ...
      if(cur->child)
      {
        ...
        if(cur->firstTest <= cur->lastTest)
        {
          if(cur->SplitType==NOM)
          {
            cur->child->firstTest =  cur->firstTest;
            cur->child->lastTest =  cur->firstTest - 1 + 
              SortNomOn(cur->NomSplit, cur->att,cur->firstTest,cur->lastTest);
          
          } else {
            SortOn(Nvar,cur->firstTest,cur->lastTest);
            cur->child->firstTest = cur->child->lastTest = cur->firstTest;
            cur->child->lastTest--;
            while(cur->child->lastTest < cur->lastTest && m[cur->child->lastTest+1][Nvar] < 0.0)
              cur->child->lastTest++;
          }
        } else {
          cur->child->firstTest = cur->firstTest;
          cur->child->lastTest = cur->lastTest;
        }

        cur->child->sib->firstTest = cur->child->lastTest+1;
        cur->child->sib->lastTest = cur->lastTest;
      }
    }
    else
    {
      ...
      {  
        if((int)m[i][Nvar-1] != cur->iClass && m[i][Nvar+3] > 0.0)
          cur->RleafCV += m[i][Nvar+3];  
      }
      if(cur->child)
      {
        cur->child->firstTest = cur->child->sib->firstTest = cur->firstTest;
        cur->child->lastTest = cur->child->sib->lastTest = cur->lastTest;
      }
    }
  ...
  while(cur)
  {
    if(cur->child)
    {
      cur->RsubTreeCV = cur->child->RsubTreeCV+cur->child->sib->RsubTreeCV;
    } else {
      cur->RsubTreeCV = cur->RleafCV;
    }
    cur = cur->nextDown();
  }
}
  ...
  if(n->IsLeaf(K)){
    if (n->parent&& n->parent->child == n)
      sprintf(buf,"%s is %s (K=%d) (%g(%g)/%d) (a=%g) T=%d\n",
                         ...
                         n->K, n->Rleaf, n->RsubTree, (n->last - n->first + 1),
                         n->alpha, n->T);
    else
    ...
    sprintf(buf+strlen(buf)," } (K=%d) (%g(%g)/%d) (a=%g) T=%d\n", 
        n->K, n->Rleaf, n->RsubTree, (n->last - n->first + 1), n->alpha, n->T);
  }
    ...
    if(n->fuzzindex)  {
      if(n->parent)  {
        if(n->parent->child == n)
          sprintf(buf,"IF fuzzification(%d) of (%g) ", n->fuzzindex,
                                                                 -(n->fSplit));
        else
          sprintf(buf,"ELSE IF fuzzification(%d) of (%g) ", n->fuzzindex,
                                                                 -(n->fSplit));
      }
    else sprintf(buf,"IF fuzzification(%d) of (%g) ",n->fuzzindex,-(n->fSplit));
    for(int i=0;i<NVarsOrdSplit;i++) {
          if(fabs(n->coeff[i]) > 0.0) {
      ...
      if(n->parent)  {
        if(n->parent->child == n)
          sprintf(buf,"IF the value of (%s) is %s", VarNames[n->att].c_str(), 
                                              n->FuzzSplit->GetName().c_str());
        else
          sprintf(buf,"ELSE IF the value of (%s) is  %s", 
                    VarNames[n->att].c_str(), n->FuzzSplit->GetName().c_str());
      }
      else
        sprintf(buf,"IF the value of %s is %s", VarNames[n->att].c_str(), 
                                              n->FuzzSplit->GetName().c_str());
...
double NomData::Derivs(Node* root, int K,int begin,int end)
{
  ...
  for(i=begin;i<=end;i++)
  {
    ...
    while(cur)
    {
      ...
      if(curIsLeaf)
      {
        double aux = cur->tempMU*m[i][Nvar+4];
        if(cur->parent) aux *= cur->parent->cumMU;
        int Class = (int)m[i][Nvar-1];
        if(i==begin)
        {
          for(int j=0; j<NumClass; ++j)
            cur->NodePop[j]  = 0.0;
          cur->MembershipTotal =0.0;
        }

        cur->NodePop[Class] += aux;
        cur->MembershipTotal += aux;
        ...
        if(i== end)
        {
          ...
          if(cur->MembershipTotal > 1.0e-10)
            aux /= cur->MembershipTotal;
          ss -= aux;
        }
        ...
        cur->child->tempMU = mu;
        cur->child->cumMU = cur->cumMU*mu;
        cur->child->sib->tempMU = (1-mu);
        cur->child->sib->cumMU = cur->cumMU*(1-mu);
      }
  ...
  for(i=begin;i<=end;i++)
  {
    ...
    while(cur)
    {
      ...
      {
        if(cur->MembershipTotal > 1.0e-10)
          cur->tempY  = -2.0*cur->NodePop[Class]/cur->MembershipTotal;
        double aux =0.0; 
        for(int j=0; j<NumClass; ++j)
          aux += cur->NodePop[j] * cur->NodePop[j];
        if(cur->MembershipTotal > 1.0e-10)
          cur->tempY +=  aux/(cur->MembershipTotal * cur->MembershipTotal);
      } else {
    ...
    while(cur)
    {
      ...
      {
        Node* temp = cur->child;
        cur->tempY = temp->tempY*temp->tempMU;
        temp = temp->sib;
        cur->tempY += temp->tempY*temp->tempMU;
      } 
...
double NomData::Error(Node* root, int K, int begin,int end)
{
  ...
  for(int i=begin;i<=end;i++) {
    ...
    while(cur)  {
      ...
      if(curIsLeaf) {
        if(Class != cur->iClass) {
          double aux = m[i][Nvar+4]*cur->tempMU;
          if(cur->parent) aux *= cur->parent->cumMU;
          ss += aux;
        }
      }
  ...
  for(i=begin;i<=end;i++)
  {
    ...
    while(cur)
    {
      ...
      if(curIsLeaf)
      {
        ...
        if(i==begin)
        {
          cur->storage = new double*[1];
          cur->storage[0] = new double[NumClass];
          for(int j=0; j<NumClass; ++j)
            cur->storage[0][j] = cur->NodePop[j]  = 0.0;
          cur->MembershipTotal =0.0;
        }
        ...
        {  
          cur->norm =0.0;
          for(int j =0; j<NumClass;++j)
            cur->norm +=pow(cur->NodePop[j],alpha);
        }
  ...
  for(i=begin;i<=end;i++)
  {
    ...
    while(cur)
    {
      ...
      if(curIsLeaf)
      {
        ...
        if(i == begin)
        {
          if(aux > 1e-10)
          {
            for(int j =0; j<NumClass; ++j)
            {
              ...
              {
                double aux0 = ((k==j) - pow(cur->NodePop[k],alpha)/cur->norm);
                cur->storage[0][j] += aux0*aux0;
              }
        ...
        ss += aux*cur->storage[0][Class];
        if(i==end)
        {
          delete[] cur->storage[0];
          delete[] cur->storage;
        }
      } else {
        double mu = cur->ParamMem(m[i],NVarsOrdSplit);
  ...
  for(i = begin; i<=end; ++i)
  {
    ...
    while(cur)
    {
      ...
      //    ASG: NOT GOOD
        for(j=0;j<NumClass; ++j)
        {
           tempPop[j] += aux*cur->NodePop[j]/cur->MembershipTotal;          
        }
  ...
  while(cur) {
    ...
    switch(cur->SplitType) {
      ...
      case ORD:
        ...
        if(cur->coeff && !cur->univariate_split) { //IsUnivariateSplit()) {
          for(int j=0;j<NVarsOrdSplit;j++) {
...
Data::Data(DataSet* ds/*1, CConsole* pt*/,void(*_console)(char*))
{
//1  console = pt;
  console = _console;
  name = "";
  ...
  Nfuzz = ds->GetNumFuzz();
  Nordfuzz = Nord+Nfuzz;
  NVarsOrdSplit = ds->GetNumVarsOrdSplit();
...
//  double amax,amin;
  ...
  for(i=0;i<NTotal;i++)
  {
    ...
    {
      if ((j>=Nordfuzz&& j<Nordfuzz+Nnom) || (j==Nvar-1 && DepVarNom))
        m[i][j] = ds->GetValueVar(i,j) + 0.5; // assure that casting to int for nomvars yields correct values
      else m[i][j] = ds->GetValueVar(i,j);
...
{
  //Matriz de datos
//  printf("Resizeing from %d to %d\n", NTotal, ndatos);
//  printf("Nvar = %d\n", Nvar);
  double **mm = new double*[ndatos+100];
  //Reservamos 1 de mAs, no sE si en algUn sitio se pasan
  for(int i=0;i<100;i++) mm[ndatos + i] = 0;
  int min = NTotal<ndatos ? NTotal : ndatos;
  for(int i=0;i<min;i++) mm[i] = m[i];
  if (ndatos>NTotal) { //Ampliamos el chiringo
//    printf("ndatos>NTotal (Nvar= %d)\n", Nvar);
...
//    printf("reducimos\n");
    for(int i=ndatos;i<NTotal;i++) delete []m[i];
  }
  if (m) delete []m;
  m = mm;
  NTotal = ndatos;
  if (NTotal!=NSel+NTest+NTrain) SetNTrain(NTotal/2);
}
                                              ...
                                              vector<bool> *attributes_to_use)
{
  ...
  *chiBest = -1;
  ...
  for(i=0;i<NVarsOrdSplit;i++) {
    ...
    iSplit=-1;
    chi = FindOrdSplit(scfirst, n->last, i, Nmin,n->MembershipTotal, &iSplit);
  ...
  for(i = Nord; i< Nordfuzz; ++i) {
    ...
    chi = FindFuzzSplit(scfirst, n->last, i, Nmin, n->MembershipTotal, 
                                                                   &FuzzSplit);
  ...
  for(i=Nordfuzz;i<Nvar-1;i++) {
    if (attributes_to_use && !((*attributes_to_use)[i])) continue;
    int NomSplit = 0;
    chi = FindNomSplit(scfirst,n->last, i, Nmin, n->MembershipTotal,&NomSplit);
  ...
  if(multsplits && iSplit >=0) {
    ...
    if(n->coeff) {
      ...
      FindOrdSplit(scfirst,n->last,Nvar, Nmin,n->MembershipTotal, &iSplit);
      if(iSplit >= 0) {
        int iS2 = iSplit+1;
        ...
        chi = FindMultiSplit(scfirst,n->last, Nmin,n->MembershipTotal, use,
                                             n->coeff, &(n->fSplit), &tempmin);
        EliminateVariables(scfirst, n->last, Nmin, n->MembershipTotal, 
  ...
  else if (n->SplitType ==ORD || (n->SplitType == FUZZ && n->fuzzindex)) {
    n->fSplit = oldSplit;
    if(n->coeff) {
      for(i=0;i<NVarsOrdSplit;i++) {
  ...
  n->CrispFlag = (n->SplitType != FUZZ) && 
                            (n->parent == NULL ? true :  n->parent->CrispFlag);
  if(n->CrispFlag) {
    if(n->SplitType == ORD) {
      // Sort
  ...
  for(i=0;i<NVarsOrdSplit;i++)
  {
    ...
    *chiBest = -1;
    ...
    if(iSplit >=0 && (*chiBest < 0.0 || chi < *chiBest))
    {
      ...
      double p1 = min_in_child/n->MembershipTotal;
      double p2 = 1.0 - p1;
      AvChi += chi;

      GainRatio[i] = (n->entropy - chi)/((n->MembershipTotal) * (- p1*log(p1) - p2*log(p2)));
...
/*  for(i = Nord; i< Nordfuzz; ++i)
  {
    ...
    FuzzySet* FuzzSplit = tempprop->Types;
    chi = FindFuzzSplit(scfirst, n->last, i, Nmin, n->MembershipTotal, &FuzzSplit);
  ...
  */
  ...
  if(multsplits && iSplit >=0) // Do only if there is an ordinal split. If correct, always
  {
    ...
    if(n->coeff)
    {
      ...
      if(iSplit >= 0)
      {
        ...
        n->fSplit = 0.5*(m[iSplit][Nvar]+m[iS2][Nvar]);
        tempmin = min_in_child;
        chi = FindMultiSplit(scfirst,n->last, Nmin,n->MembershipTotal, use, n->coeff, &(n->fSplit), &tempmin);
        EliminateVariables(scfirst, n->last, Nmin, n->MembershipTotal, n->coeff,  use, chi,&(n->fSplit), &tempmin);
        for(i=n->first;i<=n->last;i++) m[i][Nvar] -= n->fSplit;
        for(i=0;i<NVarsOrdSplit;i++)
        {
          n->fSplit -= n->coeff[i]*vshift[i];
  ...
  n->CrispFlag = (n->SplitType != FUZZ) && (n->parent == NULL ? true :  n->parent->CrispFlag);
//  n->CrispFlag = false;
  if(n->CrispFlag)
  {
  ...
  while(cur->parent)
  {
    for(int i= n->first; i<= n->last; ++i)
    {
      ...
      case ORD:
        ...
        if(cur->parent->coeff && !cur->parent->univariate_split /*IsUnivariateSplit()*/)
        {
          for(int j=0;j<NVarsOrdSplit;j++)
...
printf("MS<-------------%g--------%g-------------------------\n", m[i][Nvar], m[i][cur->parent->att]-cur->parent->fSplit);*/
  ...
  /*      int iX = cur->parent->att;
        ...
        if(cur->sib)
        {
          m[i][Nvar+3] *= cur->parent->FuzzSplit->Membership((m[i][iX]- Gvshift[iX])/Gvscale[iX]);
        }
        else
        {
          m[i][Nvar+3] *= (1.0 - cur->parent->FuzzSplit->Membership((m[i][iX]- Gvshift[iX])/Gvscale[iX]));
        }
    */
        break;
        default:
...
/*  FuzzySet* newfz = new FuzzySet("new",SHOULDER_DEC);
  ...
  for(int i=1; i<=10; ++i)
  {
    ...
    double chi = FindFuzzSplit(first, n->last, n->att,  Nmin, n->MembershipTotal, &fzsplit);
    chi = 2.0*chi/2.0;
    if(chi>=0.0 && chi<chiBest)
    {
      chiBest = chi;
      if(n->fuzzindex) delete n->FuzzSplit;
      n->FuzzSplit = new FuzzySet(fzsplit); // ASG Danger: new objects that are not destroyed
      n->fuzzindex = i;
      n->min_in_child = min_in_child;
    }
  }
...
//------------------------------------------------  GetBootstrapSample  -----
//---------------------------------------------------------------------------
Data* Data::GetBootstrapSample(int N, bool Weighted, bool IncludeOobAsTest)
{
...
Data* NomData::GetBootstrapSampleStratified(int N)
{
  ...
  for(int i = 0; i < NumClass; i++) {
    ...
    Data *d2 = d->GetBootstrapSample( n );
...
//---------------------------------------------------------------------------
Data* MINomData::GetBootstrapSample(int N, bool Weighted, bool IncludeOobAsTest)
{
  Data *bootstrap = Clone(0, 0);
...
void MINomData::AddData(Data *d2, int ini, int fin)
{
  ...
  int igrp = NMITotal;
  int last_value =  m[ ntotalprevio ][ Nvar + 6 ];
  for(int i = ntotalprevio; i < NTotal; i++) {
    if (last_value !=  m[ i ][ Nvar + 6 ] ) igrp++;
