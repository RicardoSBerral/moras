...
{
  if (left+right+1>=min_cost.size()) {
    min_cost_left.push_back(-1);
    min_cost_right.push_back(-1);
    min_cost.push_back(-1.);
	min_cost_alpha.push_back(-std::numeric_limits<double>::infinity());
  }
...
void Tree::BuildRandomForestMember(Data* data,int Nmin, int num_random_vars )
{
  ...
  vector<int> atts(data->GetNumVar()-1);
  for (unsigned i=0;i<atts.size();i++) {
    atts[i] = i;
  }
  while(cur) {
    cur->MembershipTotal = data->FindMembership(cur);
    if(cur==root) data->FixLeaf(cur);
    if (cur->last-cur->first+1 > Nmin && cur->Rleaf > 0.0) {
       vector<bool> use(data->GetNumVar()-1, false);
       for (unsigned i=0;i<num_random_vars;i++) {
...
void Tree::BuildDecisionStump(Data* data,int Nmin)
{
  ...
  root->MembershipTotal = data->FindMembership(root);
  data->FixLeaf(root);

  if((root->last - root->first + 1) > Nmin && root->Rleaf > 0.0) {
...
//      if( cur->MembershipTotal >= 2*Nmin && cur->Rleaf > 0.0) // Original
//      if( !cur->DoesntWantSplitting && (cur->last-cur->first+1) >= 2*Nmin &&  //Changed by GMM
//                                                               cur->Rleaf > 0.0)
      if( !cur->DoesntWantSplitting && (cur->last-cur->first+1) > Nmin &&  //Changed by GMM
                                                               cur->Rleaf > 0.0)
...
void Tree::CreateAndFillChildren(Node* n, int last, Data* data, int Nmin)
{
  ...
  n1->MembershipTotal = data->FindMembership(n1);
  data->FixLeaf(n1);
  if(n1->MembershipTotal < Nmin)
  {
    (*console)("Error: CreateandFillChildren0");
//1    console->printf("Error: CreateandFillChildren");
  }
  n2->MembershipTotal = data->FindMembership(n2);
  data->FixLeaf(n2);
  if(n2->MembershipTotal <Nmin)
  {
    (*console)("Error:CreateandFillChildren1");
//1    console->printf("Error:CreateandFillChildren");
  }

  if(fabs(n->MembershipTotal - n1->MembershipTotal - n2->MembershipTotal) > 1e-6)
  {
    (*console)("Error: CreateandFillChildren2 ");
//1    console->printf("Error: CreateandFillChildren ");
  }

  if(fabs(min( n1->MembershipTotal, n2->MembershipTotal) - n->min_in_child) > 1e-1)
  {
    (*console)("Error: CreateandFillChildren3 ");
//1    console->printf("Error: CreateandFillChildren ");
  }

  n1->RsubTree = n1->Rleaf;
  n2->RsubTree = n2->Rleaf;

  if(n)
  {
    double deltaR = n1->Rleaf+n2->Rleaf-n->RsubTree;

    while(n){
      n->RsubTree += deltaR; n->T++;
      n = n->parent;
    }
...
//S Obtener familia MCT de arbol this
    while(root->T > 1) //S for i=->n_alphas <- pero tenemos todas, no las que nos interesan
    {
      Prune(alpha,K);  // Construct T_K
      ...
      if(root->T > 1)
      {
        ...
        {
          if(aux[i]->root->T > 1) aux[i]->FindAlpha(aux[i]->Ndata, -1, UseSqrt);//S Sobra
          aux[i]->Prune(alpha_mean);//S Queda igual ??
          data->AccumulateErrorCV(aux[i]->root);//S Queda igual ??

        }
        data->GetErrorCV(&RCV,&SE);
  
      } else RCV =  root->Rleaf/data->GetNGrow();
      ...
      if(RCV <= Rbest+ SEaux*SEbest)
      {
        ...
        if(depvarnom)
          RMSres = root->RsubTree;
        else RMSres= sqrt(root->RsubTree/data->GetNGrow())/data->GetYGvscale();
      }
//fprintf(tablas, "%5g  %5d, ", root->RsubTree, root->T);//SG
//arb[0]='\0';
//Write(data, K, arb);
...
//printf("------------------------- K=%d \n", K);
      sprintf(kk,"K = %d Size = %d R = %g RCV = %g (%g) alpha = %g", K, 
                               root->T,root->RsubTree/root->Rleaf,RCV,SE,alpha);
      (*console)(kk);
//Write(data);
  ...
  //GMM          K,root->T,root->RsubTree/(data->GetNGrow()),RCV,SE,alpha, ss);
  //GMM        (*console)(kk);
  ...
  //        sprintf(kk, "K = %d  Size = %d  R = %g  RCV = %g (%g)  alpha = %g  Rs = %g  RMS = %g \n  NormRMS = %g ",
    //        K,root->T,root->RsubTree/(data->GetNGrow()*factor),RCV/factor,SE/factor,alpha,ss*ss/factor,ss/sqrt(factor), ss/(sqrt(factor)* ((OrdData*)data)->GetRMSTest()));
      //    (*console)(kk);
        }
      }
      else
      ...
      (*console)("Hola1");
          sprintf(kk, "K = %d  Size = %d  R = %g  RCV = %g (%g)   alpha = %g ",
            K,root->T,root->RsubTree/(data->GetNGrow()),RCV,SE,alpha);
          (*console)(kk);
...
"alpha = %g ", K,root->T,root->RsubTree/(data->GetNGrow()*factor),
...
void Tree::PruneMCT(Data* data, int Nmin, bool SE_0)
{
  ...
  SelectMinimumCostTrees(trees); // Esto actualiza las K
  while(root->T > 1)
  {
    Prune(alpha,K);  // Construct T_K
    ss = data->Score(-1,-1,root,K);
    ...
    if(root->T > 1)
    {
      ...
      alpha_prev = alpha; alpha = FindAlpha(Ndata, K, true); // Calcula el alpha con la raiz
      alpha_mean = sqrt(alpha_prev*alpha);
      data->InitialiseErrorCV();
      for(int i=0;i<data->GetNumCV();i++)
      {
...
void Tree::Prune(double alpha, int K)
{
  ...
  while(cur)
  {
    if(cur->child && (K < 0 || cur->child->K < 0))
      if(fabs(cur->alpha - alpha) < EPS2 || cur->alpha < alpha)
        FreeSubTree(cur, K);
    cur = cur->nextDown(K);
  }
}
...
void Tree::Prune2(double alpha, int ndata, int K, bool ComoArticulo)
{
  ...
  while(cur)
  {
    cur->RPrune = cur->Rleaf + alpha*ndata;
    cur = cur->nextDown(K);
  }
  cur = root;
  while(cur->child && (K < 0 || cur->child->K < 0)) cur = cur->child;
  ...
  {
    if (cur->IsLeaf(K)) {
      cur->RPruneSubTree = cur->RPrune;
      for(int jj=0;jj<cur->depth;jj++) fprintf(kk, "  ");
      fprintf(kk,"ESub=%g  RNodo=%g <-----\n", cur->RPruneSubTree, cur->RPrune);
    }
    else {
      bool HayQuePodar;
      cur->RPruneSubTree = cur->child->RPruneSubTree +
                                                 cur->child->sib->RPruneSubTree;
      if (ComoArticulo) {
        //the branch is pruned if the error of its leaves is equal or bigger
        HayQuePodar =fabs(cur->RPrune - cur->RPruneSubTree) < EPS2 ||
                                              cur->RPrune <= cur->RPruneSubTree;
      }
      else {
        //the branch is pruned if the error of its leaves is bigger
        HayQuePodar = fabs(cur->RPrune - cur->RPruneSubTree) > EPS3 &&
                                               cur->RPrune < cur->RPruneSubTree;
      }
      if(HayQuePodar) {
        fprintf(kk,"----------------CUT----------------\n");
  ...
  while(cur) {
    if (cur->IsLeaf(-2)) {
      if (cur->IsLeafAfterPrune)
        cur->DoesntWantSplitting = true;
      else
        cur->IsLeafAfterPrune = true;
    }
    cur = cur->nextDown(-2);
  }
 fclose(kk);
...
//--------------------------------------------------------------------------
void Tree::SelectMinimumCostTrees(std::map<Node*, PruneInfo*> min_cost_trees) 
{
  ...
  kl++;
  UpdateMinimumCostTrees(min_cost_trees, root, kl-1);
  while (kl > 1) {
    alpha = std::numeric_limits<double>::infinity();
    for (int k = kl -1 ; k >= 1 ; k--) {
      new_alpha = (pi->min_cost[k-1] - pi->min_cost[kl-1])/ (sqrt(kl) - sqrt(k));
      if (new_alpha <= alpha) {				
        alpha = new_alpha;
        new_kl = k;				
      }
    ...
    UpdateMinimumCostTrees(min_cost_trees, root, new_kl-1);
    kl = new_kl;
  }
  root->K = -1;
}
  ...
  for(int i = maxdepth; i >=0; --i)
  {
    ...
    while(cur)
    {
      ...
      if(cur->depth == i)
      {
        ...
        {
          Node* temp = cur->child;
          cur->RPruneSubTree = temp->RPruneSubTree + temp->sib->RPruneSubTree;
        }

        if(!curIsLeaf && cur->RPrune <= cur->RPruneSubTree)
        {
          FreeSubTree(cur,K);
          cur->RPruneSubTree = cur->RPrune;
          curIsLeaf = true;
...
void Tree::FreeSubTree(Node* n, int K)
{
  ...
  if(K<0) n->child = NULL;
  double deltaR = n->Rleaf - n->RsubTree;
  double deltaRCV = n->RleafCV - n->RsubTreeCV;
  int deltaT = 1 - n->T;
  double deltas1 = n->s1leaf - n->s1;
  double deltas12 = n->s12leaf - n->s12;
  while(n)
  {
    n->RsubTree += deltaR; n->RsubTreeCV += deltaRCV;
    n->T += deltaT;
    n->s1 += deltas1; n->s12 += deltas12;
    n = n->parent;
  }
}
...
/*    cur->first = begin;
    ...
    cur->RsubTree = nerror = ThrowData(data, begin, leftLast, K, cur->child) +
